...
import { SqlColumn, SqlReference, SqlManyToManyRelationship, SqlTable, SqlKeyType, SqlDatabaseSchema } from '~/sql'
 ...
 * @param options
 */
export async function showDatabases(options: { connection: knex }): Promise<string[]> {
 ...
 * @param options
 */
export async function showTables(options: { connection: knex; database: string }): Promise<string[]> {
 ...
 * @param options
 */
export async function showIntermediateTables(options: { connection: knex; database: string }): Promise<string[]> {
  const { connection, database } = options
  return (await connection.raw(`
 ...
 */
function generateBooleanFieldSchema(defaultValue?: string | number | number): JsonSchema {
  return {
    type: 'boolean',
 ...
 */
function generateIntegerFieldSchema(parsedFieldType: string[], defaultValue?: string | number): JsonSchema {
  ...
  const calculateBounds = (): { maximum: number; minimum: number } => {
  ...
  return {
    type: 'integer',
    default: defaultValue !== undefined ? parseInt(defaultValue as string) : undefined,
    ...calculateBounds(),
  }
}
 ...
 * @param parsedFieldType
 */
function generateNumberFieldSchema(defaultValue?: string | number): JsonSchema {
  return {
    type: 'number',
    default: defaultValue !== undefined ? parseFloat(defaultValue as string) : undefined,
  }
}
 ...
 * @param parsedFieldType
 */
function generateStringFieldSchema(parsedFieldType: string[], defaultValue?: string | number): JsonSchema {
  const [type, size] = parsedFieldType
  const base64len = (bytes: number): number => 4 * (bytes / 3)
  const generateProps = (): JsonSchema => {
    switch (type) {
      case 'tinytext':
        return {
          maxLength: 255,
 ...
 * @param parsedFieldType
 */
function generateEnumFieldSchema(parsedFieldType: string[], defaultValue?: string | number): JsonSchema {
  const [_type, options, ..._rest] = parsedFieldType
  return {
    type: 'string',
    default: defaultValue,
 ...
 * @param parsedFieldType
 */
function generateSetFieldSchema(parsedFieldType: string[], defaultValue?: string | number): JsonSchema {
  const [_type, options, ..._others] = parsedFieldType
  return {
    type: 'array',
    uniqueItems: true,
    default: defaultValue,
 ...
 * @param type
 */
function generateColumnSchema(raw: string, defaultValue?: string | number | number): JsonSchema {
 ...
 * @param options
 */
export async function getDatabaseReferences(options: { connection: knex; database: string }): Promise<SqlReference[]> {
  const { connection, database } = options
  const query = `
    USE INFORMATION_SCHEMA;
    SELECT 
...
export async function getManyToManyRelationships(options: {
  ...
  if (intermediateTables.length && intermediateTables.length >= 0) {
    return await mapAsync<string, SqlManyToManyRelationship>(intermediateTables, async (table: string) => {
...
export async function parseDatabase(options: {
  ...
  }>((r, x: MySqlColumnDescription) => {
    const { table } = x
    if (!r.hasOwnProperty(table)) r[table] = []
    r[table] = [...r[table], x]
    return r
  }, {})
  const tables: SqlTable[] = mapObject<SqlTable, MySqlColumnDescription[]>(mappedResult, (tableName, columns) => ({
    name: tableName,
 ...
 * @param options
 */
export async function parseTable(options: { connection: knex; database: string; table: string }): Promise<SqlTable> {
