...
export async function loadSchemaFromFile(filePath: string): Promise<JsonSchema> {
  ...
  const parsed = await ((): unknown =>
    new Promise((resolve): void => {
      fs.readFile(correctPath, (_err, data) => {
        const raw = data.toString()
        if (extension === 'yaml' || extension === 'yml') resolve(yaml.safeLoad(raw))
        else resolve(JSON.parse(raw))
      })
    }))()
  if (typeof parsed === 'object' && parsed && parsed.hasOwnProperty('$schema')) return parsed as JsonSchema
